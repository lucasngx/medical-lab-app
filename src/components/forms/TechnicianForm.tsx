...
export default function TechnicianForm({ initialData, onSuccess }: TechnicianFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (initialData?.id) {
        await technicianService.updateTechnician(initialData.id, formData);
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex justify-end space-x-3">
        ...
        >
          {isLoading ? 'Saving...' : (initialData?.id ? 'Update' : 'Create')} Technician
        </button>
      </div>
    </form>
  );
